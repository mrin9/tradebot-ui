    ...
    <div class="review-charts-wrapper">
        <template v-if="backtestStore.selectedBacktest">
            <div class="charts-container">
                <ReviewChartComp :instrumentId="reviewId" :timeframe="candleTimeframe"
                    :indicators="indicators" :markers="markers" :levels="levels"
                    :symbol="reviewSymbol" :maxTimestamp="maxTimestamp" />
            </div>
        </template>
...
const reviewId = computed(() => currentTrade.value?.instrumentId);

const reviewSymbol = computed(() => currentTrade.value?.instrumentDesc || 'Contract');
